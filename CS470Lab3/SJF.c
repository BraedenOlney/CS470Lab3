...
int findNextProcess(Process proc[], int current_time) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if(proc[i].arrival_time<=current_time && !proc[i].is_completed){
            ...
            if(proc[i].remaining_time==min_time){//if two processes have the same remaining time choose the one with the earlist arrival time
...
void srtf(Process proc[]) {
    ...
    while (completed != n) {
        ...
        if(index != -1){ // a process is completed

            if(prev!=index){//contentswitch 
                printf("Time %d: Process %d starts\n",current_time, proc[index].process_id);
                prev=index;
            }

            proc[index].remaining_time -= 1;
            current_time +=  1;
